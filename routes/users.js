...
router.get('/:username', authUser, requireLogin, async function(req,res,next) {
  try {
    ...
    if (user == undefined) {
      throw new ExpressError(`Username (${req.params.username}) cannot be found`, 404);
    }
...
router.patch('/:username', authUser, requireLogin, async function(req,res,next
  ...
  const acceptable = ["first_name", "last_name", "phone", "email","_token"]
  try {
    if (!req.curr_admin && req.curr_username !== req.params.username) {
      throw new ExpressError('Only that user or admin can edit a user.', 401);
    }
    for (x in req.body){
      if (!acceptable.includes(x)){
        throw new ExpressError('Please select a valid field', 401);
      }
    }
