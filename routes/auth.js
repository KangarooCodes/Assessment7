...
router.post('/register', async function(req, res, next) {
  try {
    ...
    let eTest = req.body.email
    let pTest = req.body.phone
    if (eTest[0] != ('@') && eTest.includes('@') && Number(pTest) != NaN && pTest.length == 10){
        return res.status(201).json({ token });
      } else {
      throw new ExpressError('please use a valid email/phone format',400)
    }    
  } catch (err) {
    return next(err);
  }
