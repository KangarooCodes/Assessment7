...
///////////////////////////////////////////////////////////////////////////
/// JOEY ADDED THE BELOW TESTS TO MAKE SURE FIXED BUGS ARE CHECKED ///
///////////////////////////////////////////////////////////////////////////

describe("BUG Testing", () =>{
//TESTS BUG #1
  test("Bug 1: Should return correct user information", async function () {
...
//TESTS BUG #2
  test("Bug 2: Error if invalid username is used for query param", async function () {
    const res = await request(app)
      .get("/users/notReal")
      .send({ _token: tokens.u1 });
...
//TESTS BUG #3
  test("Bug 3(part 1): Non-Admins can patch their own profiles", async function () {
    const res = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, email: "patchTest" });
    expect(res.statusCode).toBe(200);
  ...
  test("Bug 3(part 2): Non-Admins cannot make themselves admin", async function () {
    const res = await request(app)
      .patch("/users/u1")
...
//TESTS BUG #5
  test("Bug 5(part 1): Email should follow correct format", async function() {
  ...
  test("Bug 5(part 2): Phone should follow correct format", async function() {
    const response = await request(app)
      .post("/auth/register")
      .send({
        username: "new_user",
...
//TESTS BUG #6
  test("Bug 6: Needs token to be verified via jwt.verify()", async function () {
    const res = await request(app)
      .get("/users/u1")
      .send({ _token: "password@123" });
    expect(res.statusCode).toBe(401);    
  });

//Additional Test
  test("Extra: Should not allow incorrect username/password to log in", async function() {
  const res = await request(app)
    .post("/auth/login")
    .send({
      username: "u1",
...
///////////////////////////////////////////////////////////////////////////
/// JOEY ADDED THE ABOVE TESTS TO MAKE SURE FIXED BUGS ARE CHECKED ///
///////////////////////////////////////////////////////////////////////////
